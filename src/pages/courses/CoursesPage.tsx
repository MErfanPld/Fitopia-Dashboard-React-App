...
import type { Course, Sport } from '../../types/api';
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="دوره جدید">
        <div className="space-y-4">
          ...
          {sports.length > 0 ? (
            <div>
              <label className="block text-xs font-semibold text-secondary mb-1">رشته (الزامی)</label>
              <select
            ...
            <FormField label="شناسه رشته" required type="number" value={sportId} onChange={(e) => setSportId(e.target.value)} />
          )}
      ...
      <Modal isOpen={!!enrollCourse} onClose={() => setEnrollCourse(null)} title={`ثبت‌نام در ${enrollCourse?.title || ''}`}>
        <div className="space-y-4">
          <FormField label="شناسه مشتری (customer)" required type="number" value={enrollCustomer} onChange={(e) => setEnrollCustomer(e.target.value)} />
          <div className="flex justify-end gap-2">
