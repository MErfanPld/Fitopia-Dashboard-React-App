...
const emptyForm = (): GymMemberInput => ({
  ...
  membership_start: null, membership_end: null, notes: '', is_active: true,
});
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: GymMemberInput = {
        full_name: form.full_name.trim(), phone: form.phone.trim().replace(/\s+/g, ''),
        sport: form.sport, coach: form.coach, source: form.source || 'manual',
        sessions_total: form.sessions_total, sessions_remaining: form.sessions_remaining,
        sessions_used: form.sessions_used, price_paid: form.price_paid, join_date: form.join_date,
        membership_status: form.membership_status, membership_type: form.membership_type,
        membership_start: form.membership_start, membership_end: form.membership_end,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as StatusFilter)} className="rounded-xl border border-border bg-input px-3 py-2 text-sm text-ink">
          <option value="all">همه وضعیت‌ها</option>
          {MEMBERSHIP_STATUS_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value as SourceFilter)} className="rounded-xl border border-border bg-input px-3 py-2 text-sm text-ink">
          <option value="all">همه منابع</option><option value="manual">ثبت دستی</option><option value="token">توکن فیتوپیا</option>
        </select>
        <select value={fitopiaFilter} onChange={(e) => setFitopiaFilter(e.target.value as FitopiaFilter)} className="rounded-xl border border-border bg-input px-3 py-2 text-sm text-ink">
          <option value="all">همه کاربران</option><option value="fitopia">کاربر فیتوپیا</option><option value="gym">فقط باشگاه</option>
        </select>
      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'ویرایش عضو' : 'عضو جدید'}>
        <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <FormField label="منبع ثبت" isSelect value={form.source || 'manual'} options={[{ value: 'manual', label: 'ثبت دستی' }, { value: 'token', label: 'توکن فیتوپیا' }]} onChange={(e) => setForm({ ...form, source: e.target.value })} />
