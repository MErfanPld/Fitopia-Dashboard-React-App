...
export const PricesPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setItems(await pricesService.list(gymId));
    } catch (e: unknown) {
  ...
  const handleSave = async () => {
    ...
    if (!form.sport || form.monthly_price == null || form.yearly_price == null) {
      showToast('شناسه رشته، قیمت ماهانه و سالانه الزامی است.', 'warning');
      return;
    }
    setSaving(true);
    try {
  ...
  const columns: Column<GymPrice>[] = [
    { key: 'sport_name', header: 'رشته', render: (r) => <span className="text-ink font-medium">{r.sport_name || r.sport}</span> },
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title={editing ? 'ویرایش قیمت' : 'ثبت قیمت'}>
        <div className="space-y-4">
          <FormField label="شناسه رشته (sport id)" required type="number" value={form.sport || ''} onChange={(e) => setForm({ ...form, sport: Number(e.target.value) || 0 })} />
