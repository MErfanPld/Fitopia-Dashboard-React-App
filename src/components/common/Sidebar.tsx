...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        <div className="h-16 px-5 border-b border-border flex items-center justify-between shrink-0 bg-sidebar">
          ...
          {gymAccessList.length === 0 ? (
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full appearance-none bg-input border border-border text-sm text-ink rounded-lg pr-8 pl-3 py-2 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors"
              >
                {gymAccessList.map((g) => (
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-4">
          {groups.map((group) => {
            ...
            return (
              <div key={group.title}>
                <div className="px-3 pb-1.5 text-[10px] font-semibold text-muted tracking-wider">
                  {group.title}
                </div>
                    ...
                    return (
                      <NavLink
                        ...
                        style={
                          active
                            ? {
                                background: 'var(--fitopia-primary-soft)',
                                borderRight: '3px solid var(--fitopia-primary)',
                              }
                            : undefined
                        }
                        onMouseEnter={(e) => {
                          if (!active) e.currentTarget.style.background = 'var(--fitopia-primary-soft-hover)';
                        }}
                        onMouseLeave={(e) => {
        ...
        <div className="p-3 border-t border-border shrink-0">
          <div className="px-3 py-2 mb-1 rounded-xl bg-surface border border-border">
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 mt-1 rounded-xl text-sm text-danger-text hover:bg-danger-soft transition-colors"
          >
