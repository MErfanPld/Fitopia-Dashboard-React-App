...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {isMobileMenuOpen && (
        <div onClick={closeMobileMenu} className="fixed inset-0 bg-ink/40 backdrop-blur-sm z-40 lg:hidden" aria-hidden />
      )}
      <aside
        className={`fixed right-0 top-0 bottom-0 w-72 lg:w-64 bg-surface border-l border-line flex flex-col z-50
          transition-transform duration-200 shadow-sm
          ${isMobileMenuOpen ? 'translate-x-0' : 'max-lg:translate-x-full'}`}
      >
        <div className="h-16 px-5 border-b border-line flex items-center justify-between shrink-0">
          ...
          <button type="button" onClick={closeMobileMenu} className="lg:hidden text-muted p-2 hover:bg-canvas rounded-lg">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-3 py-3 border-b border-line">
          ...
          {gymAccessList.length === 0 ? (
            <div className="text-xs text-muted px-2 py-2 rounded-lg bg-canvas">دسترسی به باشگاهی ندارید</div>
          ) : (
            <div className="relative">
              ...
              <select
                ...
                className="w-full appearance-none bg-canvas border border-line text-sm text-ink rounded-lg pr-8 pl-3 py-2 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
              >
                {gymAccessList.map((g) => (
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-4">
          {groups.map((group) => {
            ...
            return (
              <div key={group.title}>
                <div className="px-3 pb-1.5 text-[10px] font-semibold text-muted uppercase tracking-wider">
                  {group.title}
                </div>
        ...
        <div className="p-3 border-t border-line shrink-0">
          <div className="px-3 py-2 mb-1 rounded-xl bg-canvas">
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2 mt-1 rounded-xl text-sm text-danger hover:bg-red-50 transition-colors"
          >
